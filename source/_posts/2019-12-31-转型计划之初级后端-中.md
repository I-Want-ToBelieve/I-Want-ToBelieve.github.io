...
9. 现在已经分析完 [nestjs-realworld-example-app][] 这个样板的基本开发逻辑， 可以开始动手改造了
...
[nestjs][] 其实已经做了这件事情， error.js 对应 https://docs.nestjs.com/exception-filters， 而 logger.js 对应 https://docs.nestjs.com/techniques/logger
  ...
  先创建好文件: `touch src/common/filters/all-exception.filter.ts`
...
10. 请求耗时不准确的原因是在 all-exception.filter.ts 文件中的这一行代码 `const requestTime = request.params.requestTime || 0`
  ...
  先新建文件 `touch src/common/interceptors/logging.interceptor.ts`
...
这个样板出了点小问题， tsc 编译报错了， 所以我准备迁移到官方的样板文件。
